  ...
  return (
    <div className="min-h-screen bg-brand-cream pt-24 pb-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Gallery & Leaderboards Link */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1 }}
          className="bg-brand-navy p-6 sm:p-8 rounded-3xl shadow-xl border border-brand-navy/90 mb-8 flex flex-col sm:flex-row items-center justify-between gap-6"
        >
          <div className="text-center sm:text-left">
            <h2 className="font-serif text-2xl text-white mb-2">Challenge Gallery & Leaderboards</h2>
            <p className="text-white/80 text-sm max-w-md">Browse all submissions, vote for your favorites, and see who's dominating the photo challenge leaderboards!</p>
          </div>
          <Link
            to="/games/photo-challenge/gallery"
            className="shrink-0 px-8 py-3.5 bg-brand-gold text-white rounded-full font-bold shadow-md hover:bg-brand-gold/90 transition-all hover:scale-105 active:scale-95 flex items-center gap-2"
          >
            <Trophy size={18} />
            View Rankings
          </Link>
                ...
                {challengeImages.length > 0 && (
                  <div className="px-6 pb-6">
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
                      {challengeImages.slice(0, 3).map((item) => {
                        const isMostLoved = item.id === popularImageId && (item.votes || 0) > 0;
                        
                        return (
                          <div key={item.id} className="group relative rounded-xl overflow-hidden bg-gray-100 flex flex-col shadow-sm border border-gray-100 aspect-square">
                            {item.type === 'video' ? (
                            ...
                            {isMostLoved && (
                              <div className="absolute top-2 left-2 z-10">
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-500 text-white shadow-md">
                                  <Trophy size={10} className="text-white" />
                                  <span className="hidden sm:inline">Most Loved</span>
                                </span>
                              </div>
                            )}
                          </div>
                    ...
                    {challengeImages.length > 0 && (
                      <div className="text-center">
                        <Link 
                          to={`/games/photo-challenge/gallery?challenge=${encodeURIComponent(challenge.title)}`}
                          className="inline-flex flex-1 w-full sm:w-auto items-center justify-center py-2.5 px-6 rounded-full border border-gray-200 text-brand-navy hover:bg-gray-50 transition-colors font-medium text-sm gap-2"
                        >
                          <ImageIcon size={16} />
                          View Gallery ({challengeImages.length})
                        </Link>
                      </div>
                    )}
                  </div>
